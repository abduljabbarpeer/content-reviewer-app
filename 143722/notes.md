...
Exception handling is **Defensive Programming**. It is the process of anticipating things that _could_ go wrong and providing a "Safety Net" so the program remains stable.
...
- **`except`**: The "Rescue" code. You should always catch _specific_ errors (like `FileNotFoundError`) rather than a general `Exception`.
